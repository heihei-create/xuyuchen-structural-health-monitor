...
public class WindowAggregator {
    ...
    public List<WindowAggregate> list(String projectId, String deviceId, String channelId, int seconds) {
        return windows.values().stream().filter(w -> w.key().projectId().equals(projectId) && w.key().deviceId().equals(deviceId) && w.key().channelId().equals(channelId) && w.key().seconds() == seconds).toList();
    }
