...
public class MeasurementIngestionService {
    ...
    public MeasurementIngestionService(DeviceService devices, MessageDeduplicationService dedup, LatestMeasurementStore latest, MeasurementHistoryStore history, MeasurementEventPublisher publisher, AlertService alerts, MeasurementMetrics metrics, DataQualityService quality, WindowAggregator windows) {
        this.devices = devices; this.dedup = dedup; this.latest = latest; this.history = history; this.publisher = publisher; this.alerts = alerts; this.metrics = metrics; this.quality = quality; this.windows = windows;
    }
    public MeasurementDtos.IngestResponse ingest(Measurement measurement) {
        ...
        if (!dedup.first(measurement.projectId(), measurement.messageId())) { metrics.duplicate(); return response(measurement, false, true, false); }
        boolean accepted = latest.putIfNewer(measurement);
